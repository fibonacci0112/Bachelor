...
namespace HoloToolkit.Unity.InputModule
{
    ...
    /// </summary>
    public class SliderScript : MonoBehaviour, IFocusable, IInputHandler, ISourceStateHandler
    {
        /// <summary>
        ...
        {
            //////////////////////// Added Fields
            sendValues = name.Split(new String[] { " " }, StringSplitOptions.None);
            upperbound = 0.45f;
            lowerbound = -0.45f;
            oldValue = 0;
            savedPosition = new Vector3[3];
            SavePosition(0);
            //////////////////////// Added Fields
        ...
        public void SavePosition(int slot)
        {
            savedPosition[slot] = new Vector3(transform.position.x, transform.position.y, transform.position.z);
        }

        public void LoadPosition(int slot)
        {
                transform.position = savedPosition[slot];
        }
        ...
        private void UpdateDragging()
        {
            ...
            if (transform.localPosition.x < upperbound)
            {
                if (transform.localPosition.x > lowerbound)
                {
                    ...
                    message.address = sendValues[0].ToLower();
                    draggingPosition.x = pivotPosition.x + (targetDirection.x * targetDistance);



                    float scale = 256 / (lowerbound - upperbound);
                    float value = (transform.localPosition.x - upperbound) * scale;

                    if (oldValue != (int)value)
                    {
                        message.values.Add(sendValues[1]);
                ...
                {
                    transform.localPosition = new Vector3(lowerbound + 0.01f, transform.localPosition.y, transform.localPosition.z);
                }
            }
            else
            {
                transform.localPosition = new Vector3(upperbound - 0.01f, transform.localPosition.y, transform.localPosition.z);
