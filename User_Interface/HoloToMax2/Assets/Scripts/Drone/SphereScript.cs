...
namespace HoloToolkit.Unity.InputModule.Tests
{
    ...
    /// </summary>
    public class SphereScript : MonoBehaviour, IFocusable, IInputHandler, ISourceStateHandler
    {
        /// <summary>
        ...
        {
            savePoints[slot] = new Vector3(transform.position.x, transform.position.y, transform.position.z);
        }

        public void LoadPosition(bool reset, int slot)
        {
            Vector3 point;
            if (reset)
            {
            ...
            else if (savePoints[slot] != null)
            {
                point = savePoints[slot];
            }
            else
            {
                return;
        ...
        private void Update()
        {
            ...
            if (activated)
            {
                ...
                message.values.Add(Int32.Parse(name.Substring(7)));

                float scale = 256 / (-9.9f - 9.6f);
                float value = (transform.localPosition.x - 9.6f) * scale;

                message.values.Add((int)value);

                scale = 256 / (5.24f - -1.34f);
                value = (transform.localPosition.y - -1.34f) * scale;

                message.values.Add((int)value);

                scale = 256 / (-5.7f - 13.8f);
                value = (transform.localPosition.z - 13.8f) * scale;
